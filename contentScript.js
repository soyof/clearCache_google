...
(function() {
    ...
    console.log('清理缓存助手内容脚本已加载:', location.href);
    ...
    const StorageCleaner = {
        ...
        clearLocalStorage() {
            try {
                ...
                localStorage.clear();
                console.log(`已清理 ${itemCount} 个 LocalStorage 项目`);
        ...
        clearSessionStorage() {
            try {
                ...
                console.log(`已清理 ${itemCount} 个 SessionStorage 项目`);
                return { success: true, count: itemCount };
            } catch (error) {
        ...
        async clearCacheAPI() {
            try {
                if ('caches' in window) {
                    ...
                    console.log(`已清理 ${cacheNames.length} 个 Cache API 缓存`);
            ...
            try {
                if ('serviceWorker' in navigator) {
                    ...
                    const unregisterPromises = registrations.map(registration => {
                        console.log('注销 Service Worker:', registration.scope);
    ...
    window.addEventListener('beforeunload', () => {
        console.log('页面即将卸载:', location.href);
    });
